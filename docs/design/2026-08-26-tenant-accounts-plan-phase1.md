...
**裁决 2 — 停用密钥在一期走 `resources.yaml` + SIGHUP。**
...
**实施时补上的一条（初稿漏了，e2e 打真流量才撞出来）：** 写完文件必须
`pkill -HUP -x aisix`。文件模式下网关**只在 SIGHUP 时**重读 `resources_file`
（`crates/aisix-server/src/main.rs`，没有文件监听）。少了这一步闸刀是空的 ——
门户尽职地写下 `disabled: true`、账面显示负余额，而客户被无限期继续服务，
又一个静默的免费推理洞。做法与 `crates/aisix-console` 一致。
...
**实施时的偏离：** 仓库既有的 `tests/e2e` harness 依赖 etcd，而 etcd 在开发机上
没有安装，跑不起来。改走网关的**文件模式**（`resources_file`）—— 不需要 etcd，
而门户本来就是写这个文件的，所以这一层测的恰好是生产里那条链路。夹具落在
`web-portal/e2e/money.fixture.ts`，只有上游 LLM 与 PromQL 引擎是垫片，网关、
定价、指标、账本、停用、重载全是生产件。
...
**实施后的修订（2026-08-26）：** Task 9 打真流量时又撞出**第三处**同类缺陷：
写完 `resources.yaml` 没有通知网关重载，闸刀是空的（见裁决 2）。三处的形状
完全一样 —— 不报错、只是少收，而客户照常被服务。
